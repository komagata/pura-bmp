  ...
  def self.run(input)
    unless File.exist?(input)
      ...
      unless generate_test_image(input)
        $stderr.puts "Error: could not generate test image. Provide an existing BMP file."
        exit 1
      end
    end
    ...
    # pura-bmp
    results << bench("pura-bmp") do
      image = Pura::Bmp.decode(input)
    ...
    results << bench("ImageMagick") do
      out = `convert #{shell_escape(input)} -depth 8 rgb:- 2>/dev/null`
      $?.success? ? out.bytesize : nil
    end
